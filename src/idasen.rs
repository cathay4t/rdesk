// Copy from https://github.com/aklajnert/idasen/blob/master/src/lib.rs

pub use btleplug::api::Peripheral as Device;
use btleplug::api::{
    BDAddr, Central, Characteristic, Manager as _, ParseBDAddrError,
...
/// convert desk response from bytes to tenth of millimeters and a speed of
/// unknown dimension
///
/// ```
    ...
    for peripheral in adapter.peripherals().await? {
        if let Some(props) = peripheral.properties().await? {
            if match mac {
                ...
                None => {
                    props.local_name.iter().any(|name| name.contains("Desk"))
                }
            } {
...
/// Get instance of `Idasen` struct. The desk will be discovered by the name. If
/// multiple are applicable a random one will be choosen.
...
pub async fn get_instance_by_mac(
    mac: &str,
    ...
    match addr {
        Ok(addr) => {
            ...
            Ok(Idasen::new(
                desks.into_iter().next().ok_or(Error::CannotFindDevice)?,
            )
            .await?)
        }
...
impl<T: Device> Idasen<T> {
    ...
    pub async fn new(desk: T) -> Result<Self, Error> {
        ...
        let control_characteristic = desk
            ...
            .ok_or_else(|| {
                Error::CharacteristicsNotFound("Control".to_string())
            })?
            .clone();
        ...
        let position_characteristic = desk
            ...
            .ok_or_else(|| {
                Error::CharacteristicsNotFound("Position".to_string())
            })?
            .clone();
    ...
    /// Move desk to a desired position. The precision is decent, usually less
    /// than 1mm off.
    pub async fn move_to(&self, target_position: u16) -> Result<(), Error> {
        self.move_to_target(target_position, None).await
    }

    pub async fn move_to_with_progress(
        &self,
        target_position: u16,
    ) -> Result<(), Error> {
        let initial_position =
            (target_position as i16 - self.position().await? as i16).abs();
        let progress = ProgressBar::new(initial_position as u64);
        progress.set_style(
            ProgressStyle::default_bar()
                .template("{spinner} {wide_bar} [{msg}cm]"),
        );
    ...
    async fn move_to_target(
        ...
        while !position_reached {
            ...
            let speed = ((moved_height as f64 / elapsed_millis as f64)
                * 1000f64) as i16;
...
//            }
            ...
            // Read last_position again to avoid weird speed readings when
            // switching direction
    ...
    /// Return the denk height in tenth millimeters and speed in unknown
    /// dimension
